class MarketdatesController < ApplicationController
  ...
  before_action :load_marketdate, only: [:create]
  load_and_authorize_resource

  # GET /marketdates
  # GET /marketdates.json
  def index
    #@marketdates = Marketdate.all.page(params[:page]).order(tradedate: :desc)
    @marketdates_A = Marketdate.where(tag: 'A').page(params[:page]).order(tradedate: :desc)
    @marketdates_G = Marketdate.where(tag: 'G').page(params[:page]).order(tradedate: :desc)
    @marketdates_M = Marketdate.where(tag: 'M').page(params[:page]).order(tradedate: :desc)
  end
  ...
  private
    ...
    def marketdate_params
      params.require(:marketdate).permit(:tag, :tradedate, :daystate)
    end
